...
function Test() {
    ...
    //카운트 함수
    function tick() {
        return setTimeout(() => IncreaseState(), 1000);
    }
    ...
    const PrintEachData = () =>{
        ...
        for(var i = 0; i < pmsDatas.length; i++){
            if(i === state){
                thisIdx = pmsDatas[i].idx
                thisData1 = pmsDatas[i].data1
                thisData2 = pmsDatas[i].data2
                thisData3 = pmsDatas[i].data3
            }
        }
        ...
        return(
            <div>
              <b>{thisIdx}</b> 
              <br/>
              <span>({thisData1})</span> <span>({thisData2})</span> <span>({thisData3})</span>
          </div>
        )
    }
...
//         <b>{user.idx}</b> 
//         <br/>
//         <span>({user.data1})</span> <span>({user.data2})</span> <span>({user.data3})</span>
//     </div>
//     )
// }